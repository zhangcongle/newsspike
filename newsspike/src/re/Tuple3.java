...
public class Tuple3 {
	...
	public Tuple(Date date, long articleId, int artOffset, OllieSentence or,
			OneOllieExtr ooe) {
		this.date = date;
		this.articleId = articleId;
		...
		lmma = or.lmma;
		deps = or.deps;
		a1 = new int[] { ooe.arg1p[0], ooe.arg1p[1], 0 };
		v = new int[] { ooe.relp[0], ooe.relp[1], 0 };
		a2 = new int[] { ooe.arg2p[0], ooe.arg2p[1], 0 };
		setArgHead(a1);
		setArgHead(a2);
		setRelHead();
		a1str = ooe.arg1;
...
//		setIndexDeps();
	...
	public List<Integer> getRelOllieIndex() {
		...
		int vhead = v[2];
		for (Entry<Integer, String> e : indexdep.row(vhead).entrySet()) {
			String type = e.getValue();
			int dep = e.getKey();
			if (dep >= a2[0] && dep < a2[1] || dep >= a1[0] && dep < a1[1]
					|| dep >= v[0]
